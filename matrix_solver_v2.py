...
def elimination(matrix, pvt=True):
    ...
    for row in range(len(in_matrix[0]) + correction):
        if in_matrix[row][pivot[0]] != 0 and row > pivot[1]:
            in_matrix = add_rows(
                ...
                row,
                pivot[1],
                -(in_matrix[row][pivot[0]] / in_matrix[pivot[1]][pivot[0]]),
            )
    ...
    for n in range(len(pivots)):
        if in_matrix[pivots[n][0]][pivots[n][1]] != 1:
            in_matrix = multiply_row(
                matrix, pivots[n][1], (1 / matrix[pivots[n][0]][pivots[n][1]])
            )
    ...
    for n in range(len(pivots)):
        for row in range(len(in_matrix)):
            if in_matrix[row][pivots[n][0]] != 0 and row < pivots[n][1]:
                in_matrix = add_rows(
                    in_matrix, row, pivots[n][0], -(in_matrix[row][pivots[n][0]])
                )
    ...
    for row in range(len(in_matrix)):
        if (
            in_matrix[row][0 : len(in_matrix[0]) - 1] == zero_row_minus1
            and in_matrix[row][len(in_matrix[0]) - 1] != 0
        ):
            in_matrix = multiply_row(
                in_matrix, row, (1 / in_matrix[row][len(in_matrix[0]) - 1])
            )
